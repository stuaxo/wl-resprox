...
//! (see docs/debugging-notes.md's 2026-07-30 entries for why that's not
//! just a coverage gap but a correctness bug: it desyncs the client's
//! new_id sequence from the compositor's). `missing_from_labwc_snapshot`
//! below is the regression test for "did we forget to wire up something
//! these crates already generated for us."
...
pub fn lookup_interface(name: &str) -> Option<&'static Interface> {
    ...
    Some(match name {
        ...
        // compositor in this table's existing coverage did -- found live
        // against the kwin container (2026-07-31, see debugging-notes.md).
        // All Gap 1 (already generated by wayland-protocols' `staging`
        // feature, just not wired in) -- see architecture-notes.md.
        ...
        // Found against mutter/gnome-shell instead (2026-07-31) -- same
        // Gap 1 pattern, staging feature already enabled.
        ...
        // not a bindable fix -- see architecture-notes.md's Gap 2 list and
        // the 2026-07-31 mutter entry in debugging-notes.md. Confirmed
        // live it's safe to keep dropping: gtk4-demo never needs it, full
        // toplevel chain still comes up and survives a crash/reconnect.
...
mod tests {
    ...
    /// this proxy (2026-07-30, see docs/debugging-notes.md) must resolve.
    /// This is the test that would have caught the original gap: a bare
    /// `cargo test` run, no live compositor needed, fails loudly the
    /// moment a real desktop's global list outpaces this table again --
    /// instead of the silent per-message drop that led to the
    /// new_id-gap/"invalid arguments" bug in the first place.
    #[test]
    ...
    /// for kwin/Plasma 6 (6.4.5, `--virtual` backend) -- Phase 9's second
    /// per-WM container. A different protocol family than labwc/sway's
    /// wlroots, so a distinct set of staging protocols showed up that
    /// wlroots compositors never advertised (2026-07-31, see
    /// debugging-notes.md).
    #[test]
    ...
    /// -- Phase 9's third per-WM container. Only one new resolvable gap
    /// this time (`wp_commit_timing_manager_v1`); the other new global
    /// mutter advertises, `gtk_shell1`, is a permanent Gap 2 (see the
    /// module doc and architecture-notes.md) and deliberately has no
    /// entry here to test for.
    #[test]
