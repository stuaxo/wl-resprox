...
pub fn lookup_interface(name: &str) -> Option<&'static Interface> {
    ...
    use wayland_protocols_plasma::appmenu::client::__interfaces as appmenu;

    // Real Qt/KDE clients bind these too, not just kwin itself -- found
    // live 2026-08-11 running scripts/qt/dmabuf_gl.py against a real kwin
    // container. Both Gap 1 (already generated by wayland-protocols'
    // `staging` feature, already enabled in this crate's own dependency --
    // just not wired in here yet, same pattern as xdg_wm_dialog_v1/
    // wp_color_manager_v1 above).
    use wayland_protocols::xdg::toplevel_drag::v1::client::__interfaces as toplevel_drag;
    use wayland_protocols::wp::pointer_warp::v1::client::__interfaces as pointer_warp;
    ...
    Some(match name {
        ...
        "org_kde_kwin_appmenu_manager" => &appmenu::ORG_KDE_KWIN_APPMENU_MANAGER_INTERFACE,
        // Real Qt/KDE clients advertise/bind these too -- see the `use`
        // aliases' own comment above for why they're Gap 1, not Gap 2.
        "xdg_toplevel_drag_manager_v1" => &toplevel_drag::XDG_TOPLEVEL_DRAG_MANAGER_V1_INTERFACE,
        "wp_pointer_warp_v1" => &pointer_warp::WP_POINTER_WARP_V1_INTERFACE,
        // zwp_text_input_manager_v2 (unlike v1/v3, both wired in above) is
        // a genuine, permanent Gap 2: not generated by any of this table's
        // four dependency crates at all (checked directly -- absent from
        // wayland-protocols' and wayland-protocols-misc's own XML sources,
        // 2026-08-11). Superseded by v3, which every modern text-input
        // client (including the Qt one that surfaced this gap) prefers
        // when both are advertised, same reasoning as gtk_shell1's own
        // permanent-Gap-2 note below -- deliberately no entry here.
...
mod tests {
    ...
    /// advertised. The last three (appmenu/toplevel_drag/pointer_warp)
    /// were found 2026-08-11 via a real Qt/KDE client
    /// (scripts/qt/dmabuf_gl.py), not kwin's own startup alone --
    /// zwp_text_input_manager_v2 was also observed that session but is a
    /// genuine Gap 2, see lookup_interface's own comment on it.
    #[test]
